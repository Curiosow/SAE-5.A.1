  ...
  <main class="min-h-screen bg-[#F7F7FB]">
    <div class="mx-auto max-w-[900px] px-4 sm:px-6 lg:px-8 pt-24 pb-16">
      ...
      <div class="fixed top-20 right-4 z-50 flex items-center gap-2 bg-white px-3 py-1 rounded-full shadow-sm text-xs border border-gray-100 cursor-pointer select-none">
        ...
        <div @click="isCoach = !isCoach" class="w-9 h-5 bg-gray-200 rounded-full relative transition-colors duration-300" :class="{ 'bg-gradient-to-r from-pink-500 to-purple-600': isCoach }">
      ...
      <header class="mb-8 animate-fade-in-down">
        <h1 class="text-3xl font-extrabold text-gray-900 tracking-tight">
          Objectifs <span class="inline-block text-transparent bg-clip-text bg-gradient-to-r from-pink-500 to-purple-600">Performance</span>
        </h1>
      ...
      <div class="flex gap-6 mb-6 border-b border-gray-200">
        <button @click="currentTab = 'matchs'" class="pb-3 text-sm font-bold transition-all relative" :class="currentTab === 'matchs' ? 'text-gray-900' : 'text-gray-400 hover:text-gray-600'">
          Matchs <span v-if="currentTab === 'matchs'" class="absolute bottom-0 left-0 w-full h-0.5 bg-gradient-to-r from-pink-500 to-purple-600 rounded-full"></span>
        </button>
        <button @click="currentTab = 'season'" class="pb-3 text-sm font-bold transition-all relative" :class="currentTab === 'season' ? 'text-gray-900' : 'text-gray-400 hover:text-gray-600'">
          Objectifs Saison <span v-if="currentTab === 'season'" class="absolute bottom-0 left-0 w-full h-0.5 bg-gradient-to-r from-pink-500 to-purple-600 rounded-full"></span>
        </button>
      </div>
      ...
      <div v-if="currentTab === 'matchs'" class="space-y-5">
        <div class="flex justify-center mb-6">
          <div class="bg-gray-100 p-1 rounded-xl inline-flex">
            <button @click="matchFilter = 'upcoming'" class="px-4 py-1.5 text-xs font-bold rounded-lg transition-all" :class="matchFilter === 'upcoming' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'">À venir</button>
            <button @click="matchFilter = 'past'" class="px-4 py-1.5 text-xs font-bold rounded-lg transition-all" :class="matchFilter === 'past' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'">Terminés</button>
          </div>
        </div>

        <TransitionGroup tag="div" name="list" class="relative space-y-5" appear>
          <div v-for="(match, i) in filteredMatches" :key="match.id || i" class="bg-white rounded-[18px] shadow-[0_10px_30px_rgba(0,0,0,0.04)] border border-gray-50 overflow-hidden transition-all duration-300 hover:shadow-lg w-full" :class="{ 'ring-1 ring-purple-100': expandedMatchId === match.id }" :style="{ transitionDelay: `${i * 50}ms` }">

            <div @click="toggleMatch(match.id)" class="p-5 sm:p-6 cursor-pointer flex flex-col sm:flex-row sm:items-center justify-between gap-4 group select-none">
              <div class="flex items-center gap-4">
                ...
                <div>
                  <h3 class="text-lg font-bold text-gray-900 group-hover:text-purple-600 transition-colors">{{ match.adversaire_nom }}</h3>
                  <div class="flex items-center gap-2 mt-1">
                    <p class="text-xs text-gray-500 font-medium">{{ match.rencontre_conclusion_info_date_match ? formatDate(match.rencontre_conclusion_info_date_match) : 'Date à définir' }}</p>
                    ...
                    <span v-if="matchFilter === 'past'" class="ml-2 text-xs font-bold text-gray-800 bg-gray-100 px-2 py-0.5 rounded">{{ match.rencontres_info_equipe1_score }} - {{ match.rencontres_info_equipe2_score }}</span>
                  </div>
                </div>
              </div>
              <div class="flex items-center justify-between sm:justify-end gap-4 w-full sm:w-auto mt-2 sm:mt-0">
                ...
                <div class="w-8 h-8 rounded-full bg-gray-50 flex items-center justify-center text-gray-400 group-hover:bg-purple-50 group-hover:text-purple-600 transition-all duration-300">
                  <svg :class="{'rotate-180': expandedMatchId === match.id}" class="w-5 h-5 transition-transform duration-300" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" /></svg>
                </div>
              </div>
            </div>
            ...
            <div v-show="expandedMatchId === match.id" class="border-t border-gray-100 bg-[#FAFAFC] p-5 sm:p-6 animate-fade-in">
              <div v-if="getMatchObjectives(match.id).length > 0" class="grid gap-3 sm:grid-cols-2">
                <div v-for="obj in getMatchObjectives(match.id)" :key="obj.id" class="bg-white p-4 rounded-xl border border-gray-100 shadow-sm flex justify-between items-center relative overflow-hidden group/obj">
                  ...
                  <div class="pl-2 flex-1">
                    ...
                    <div class="flex items-center gap-2 mt-1.5">
                      <span class="text-xs font-bold bg-gray-100 text-gray-700 px-2 py-1 rounded">
                        {{ formatOperator(obj.operator) }} {{ obj.target_value }}
                      </span>
                    </div>
                  </div>
                  ...
                  <div class="flex flex-col items-end gap-1">
                    ...
                    <div v-if="isCoach && matchFilter === 'past'" class="flex gap-2 mb-1">
                      <button @click.stop="updateStatus(obj.id, 'failed')" class="w-7 h-7 flex items-center justify-center rounded-full bg-red-50 text-red-400 hover:bg-red-500 hover:text-white transition-all" :class="{'bg-red-500 text-white shadow-md': obj.status === 'failed'}">✕</button>
                      <button @click.stop="updateStatus(obj.id, 'success')" class="w-7 h-7 flex items-center justify-center rounded-full bg-green-50 text-green-400 hover:bg-green-500 hover:text-white transition-all" :class="{'bg-green-500 text-white shadow-md': obj.status === 'success'}">✓</button>
                    </div>

                    <span v-else class="text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wide" :class="getStatusBadge(obj.status)">
              ...
              <div v-else class="text-center py-4 text-gray-400 text-sm italic">Aucun objectif défini pour ce match.</div>

              <button v-if="isCoach && matchFilter === 'upcoming'" @click="openAddModal(match.id, match.adversaire_nom)" class="mt-4 w-full group relative overflow-hidden rounded-xl bg-white p-[1px] transition-all hover:shadow-md">
        ...
        <div v-if="filteredMatches.length === 0" class="text-center py-10">
          ...
          <p v-if="matchFilter === 'upcoming'" class="text-sm text-purple-600 mt-2 cursor-pointer hover:underline font-bold" @click="matchFilter = 'past'">Voir les matchs terminés ?</p>
        </div>
      </div>

      <div v-else class="grid gap-4 sm:grid-cols-2 animate-fade-in">
        <div v-for="obj in seasonObjectives" :key="obj.id" class="bg-white p-5 rounded-[20px] shadow-sm border border-gray-100 flex justify-between items-center relative overflow-hidden group">
          <div class="absolute left-0 top-0 bottom-0 w-1" :class="getStatusColor(obj.status)"></div>
          <div class="pl-2 flex-1">
            <h3 class="font-bold text-gray-800 text-lg">{{ obj.title }}</h3>
            <div class="mt-3 inline-flex items-center text-sm font-bold text-gray-700 bg-gray-50 px-3 py-1.5 rounded-lg border border-gray-100">
              Cible : {{ formatOperator(obj.operator) }} {{ obj.target_value }}
            </div>
          </div>
          <div class="flex flex-col items-end gap-2">

            <div v-if="isCoach" class="flex gap-2 mb-1">
              <button @click.stop="updateStatus(obj.id, 'failed')" class="w-8 h-8 flex items-center justify-center rounded-full bg-red-50 text-red-400 hover:bg-red-500 hover:text-white transition-all" :class="{'bg-red-500 text-white shadow-md': obj.status === 'failed'}">✕</button>
              <button @click.stop="updateStatus(obj.id, 'success')" class="w-8 h-8 flex items-center justify-center rounded-full bg-green-50 text-green-400 hover:bg-green-500 hover:text-white transition-all" :class="{'bg-green-500 text-white shadow-md': obj.status === 'success'}">✓</button>
            </div>

            <span class="text-[10px] bg-purple-50 text-purple-700 px-2 py-1 rounded-full font-bold uppercase tracking-wide">Saison</span>
            <button v-if="isCoach" @click="deleteObjective(obj.id)" class="text-xs text-red-400 hover:text-red-600 opacity-0 group-hover:opacity-100 transition-opacity bg-white px-2 py-1 rounded border border-gray-100 shadow-sm">Supprimer</button>
          </div>
        </div>

        <button v-if="isCoach" @click="openAddModal(null, 'Saison')" class="bg-gray-50 border-2 border-dashed border-gray-200 rounded-[20px] flex flex-col items-center justify-center p-8 text-gray-400 hover:border-purple-400 hover:text-purple-600 hover:bg-purple-50/20 transition-all cursor-pointer min-h-[140px]">
    ...
    <div v-if="showModal" class="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      <div class="relative bg-white rounded-3xl shadow-2xl max-w-sm w-full p-8 z-10 transform transition-all duration-300">
        ...
        <div class="space-y-4">
          ...
          <div class="grid grid-cols-3 gap-3">
            <div class="col-span-1">
              <label class="text-xs font-bold text-gray-500 uppercase tracking-wide mb-1 block">Cible</label>
            ...
            <div class="col-span-2">
              <label class="text-xs font-bold text-gray-500 uppercase tracking-wide mb-1 block">Valeur</label>
        ...
        <div class="mt-8 flex gap-3">
          ...
          <button @click="saveObjective" class="flex-1 py-3 rounded-xl font-bold text-white bg-gradient-to-r from-pink-500 to-purple-600 shadow-lg shadow-purple-500/30 hover:shadow-purple-500/50 transform active:scale-95 transition-all text-sm">Valider</button>
        </div>
      </div>
    </div>
  </Transition>
...
async function updateStatus(id: number, status: string) {
  try {
    const res = await fetch(`${apiUrl}/objective/${id}/status`, {
      method: 'PATCH',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({ status })
    });
    if(res.ok) {
      const obj = allObjectives.value.find(o => o.id === id);
...
async function saveObjective() {
  ...
  const payload = {
    match_id: Number(form.value.matchId) || 0,
  ...
  try {
    ...
    if(res.ok) {
      ...
      alert("Erreur serveur : Vérifiez que l'ID du match est bien accepté par la base de données.");
    }
  } catch(e) { console.error(e); }
}
...
const seasonObjectives = computed(() => {
  return allObjectives.value.filter(o => o.type === 'season' || !o.match_id || o.match_id === 0);
});
...
function toggleMatch(id: any) { expandedMatchId.value = expandedMatchId.value === id ? null : id; }
function openAddModal(matchId: any, name: string) { form.value = { matchId, targetName: name, title: '', operator: '<', targetValue: '' }; showModal.value = true; }
function closeModal() { showModal.value = false; }
function formatDate(d: string) { if(!d) return ''; return new Date(d).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' }); }
...
.list-leave-active { position: absolute; width: 100%; z-index: -1; left: 0; top: 0; }
</style>