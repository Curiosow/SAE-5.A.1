<template>
  <main class="min-h-screen bg-[#f7f7fb] pt-16">
    <div
        class="mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-12 transition-all duration-300 max-w-[1180px]"
    >
            ...
            <div class="grid grid-cols-[auto_1fr] gap-6 items-start">
              ...
              <div class="flex flex-col gap-4 w-full">
                <div class="relative">
                  ...
                  >
                    {{ view.player?.last_name ?? '' }} {{ view.player?.first_name ?? '' }}
                  </h1>
                </div>
...
const displayedViews = computed(() => {
  return player.value ? [{ index: 0, playerIndex: 0, player: player.value }] : []
})
...
// Récupère uniquement la joueuse connectée
async function fetchCurrentPlayer() {
  try {
    const email = typeof window !== 'undefined' ? localStorage.getItem('email') : null
    if (!email) throw new Error('Email manquant dans le localStorage')
    ...
    const found = plD.find((p: any) => p.email === email)
    if (!found) { player.value = null; return }

    const pEv = allEv.filter((ev: any) => ev.joueuse === found.nom_csv)
    const posObj = pos.find((px: any) => px.player_id === found.id)
    player.value = { ...found, events: pEv, stats: computePlayerStats(pEv, posObj?.abrevation || ''), position: posObj?.abrevation || '' } as Player
  } catch (e) {
    console.error(e)
    player.value = null
  }
}
onMounted(() => { fetchCurrentPlayer() })
</script>
